...
class PresentationActivity3Test {
    ...
    fun successfulDisplay() {
        val i =
            Intent(ApplicationProvider.getApplicationContext(), PresentationActivity3::class.java)
    ...
    fun successfulApprovalButton() {
        val i = Intent(ApplicationProvider.getApplicationContext(), PresentationActivity3::class.java)
        ActivityScenario.launch<PresentationActivity3>(i).use {
            Intents.init()
            val intent = Intent()
    ...
    fun successfulSlideRight() {
        val i = Intent(ApplicationProvider.getApplicationContext(), PresentationActivity3::class.java)
        ActivityScenario.launch<PresentationActivity3>(i).use { activityScenario ->
            Intents.init()
            val intent = Intent()
            val intentResult = Instrumentation.ActivityResult(Activity.RESULT_OK, intent)
            Intents.intending(IntentMatchers.anyIntent()).respondWith(intentResult)
